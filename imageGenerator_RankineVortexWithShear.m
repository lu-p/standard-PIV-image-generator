...
resx=200; % image resolution x [px]
resy=150; % image resolution along y ]px]
...
Ifin1=zeros(n_imagepairs,resy,resx);
Ifin2=zeros(n_imagepairs,resy,resx);

%% 2D VELOCITY FIELD: RANKINE VORTEX WITH SHEAR

% vortex (central) + shear (y axis)
...
dp=1.5+2.5*rand(n_particles,n_imagepairs);% effective particle image diameter [pixel]

str_imp=num2str(n_imagepairs);
name=sprintf('ig_rkvs_%dx%d_%s_noise_rhocost_div%d_dec%d_R1=%d_J=%f_ok',resx,resy,str_imp, div_f, dec, R1, J)
